...
app.post('/api/baccarat-deal', async (req, res) => {
    ...
    try {
        ...
        // 🎯 [ব্যাকারাত মাস্টার - মেগা কিলার জিরো-ডাবল-ডেবিট ও টাইমআউট প্রুফ বর্ম ভাই ভাই]
        // অন্দর বাহারের ওরিজিনাল মেকানিজম অনুযায়ী রেসপন্স অবজেক্ট ডাটা একুরেট লক করা হলো ওস্তাদ
        let winAmount = 0;
        ...
        if (finalStatus === "win") {
            ...
            dbAction = "win";
            dbAmount = parseFloat(winAmount); // জিতলে উইনিং এماউন্ট যাবে
        } else {
        ...
        // 🛫 ৩. মেইন সাইটের সিকিউরড গেটওয়েতে রিয়েল-টাইম উইন-লস সেটেলমেন্ট এপিআই হিট (অন্দর বাহার সিঙ্ক লক)
        const response = await axios.post(`${MAIN_SITE_URL}/api_callback.php`, phpPayload, { timeout: 45000 });

        if (response.data && response.data.status === "ok") {
            io.emit("balanceUpdate", { username: userId, balance: response.data.balance });
            
            // 🎯 [মেগা সাকসেস ডাটা রিটার্ন]: অন্দর বাহারের ওরিজিনাল সাকসেস স্ট্রাকচার রিমিক্সড লক ওস্তাদ!
            return res.json({
                success: true,
